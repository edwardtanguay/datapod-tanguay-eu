 ...
 */
export const wrapFoundSearchWordsWithClassElement = (
	text: string,
	searchText: string,
	minimum: number = 3,
	className: string = "searchHighlight"
) => {
	...
	const searchWords = escapedSearchText
		.split(" ")
		.filter((word) => word.length >= minimum);
	if (searchWords.length > 0) {
		const regexSearchString = searchWords
			.map((word) => `(${word})`)
			.join("|");
		const regexReplaceString = searchWords
			.map((_m, i) => "$" + String(i + 1))
			.join("");
		const re = new RegExp(regexSearchString, "ig");
		const result = text.replace(
			re,
			`<span class="${className}">${regexReplaceString}</span>`
		);
...
export const parseMarkDown = (
	...
	r = qstr.replaceAll(
		r,
		'<a href="mailto:git@github.com">git@github.com</a>',
		"git@github.com"
	);
